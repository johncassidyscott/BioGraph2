...
def ingest_form4():
    ...
    for filing_id, issuer_id, accession, url in filings:
        ...
        if resp.status_code == 404:
            fail_count += 1
            if accession not in unique_404s:
                unique_404s.add(accession)
                logging.warning(f"Failed to fetch XML for {accession} (status 404)")
            continue
    ...
    cur.close()
    conn.close()
    print(f"\nSummary: {success_count} transactions inserted, {fail_count} failed, {len(unique_404s)} unique 404s.")
    logging.info(f"Summary: {success_count} transactions inserted, {fail_count} failed, {len(unique_404s)} unique 404s.")
